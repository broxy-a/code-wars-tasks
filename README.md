...
В этой задаче требуется написать функцию, которая возвращает массив строк в котором первая буква всегда заглавная, а следуюющие строчные.

String[] names = {"artorias", "john", "KELVIN", "annA", ""} -> String[] result = Artorias, John, Kelvin,Anna

🔹 Основные моменты:
    📌 Создает новый массив для хранения результата.
    📌 Проверяет массив на пустоту
    📌 Использует substring для перевода в вверхний регистр.
    📌 Использует concat для объденения части строк