...
Этот метод проверяет, является ли строка ровно одной цифрой (0-9
...
🔹 Основные моменты:
    📌 Регулярное выражение \\d для проверки цифрового символа
    📌 Явный возврат boolean вместо неявного преобразования
    📌 Регулярное выражение [0-9] - компактная проверка на ровно один цифровой символ


## 🔠 Replace all vowel

Этот код заменяет все гласные буквы (a, e, i, o, u в обоих регистрах) в строке на восклицательные знаки.
...
🔹 Основные моменты:
    📌 Используется StringBuilder для эффективной конкатенации строк
    📌 Строка vowel содержит все гласные в обоих регистрах
    📌 Иммутабельность: исходная строка (final) не изменяется
    📌 Метод indexOf() проверяет принадлежность символа к гласным
